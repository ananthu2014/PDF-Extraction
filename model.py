...
'''Pytesseract is an Optical character Recognition(OCR) library to extract text data from images'''
...
'''This function is used to parse through the extracted text using Regular expression'''
def safe_extract(regex, text, group_num=1, default=None):
    match = re.search(regex, text)
    return match.group(group_num) if match else default

'''The regex pattern is defined here. This needs to be updated in case of different Invoices'''
...
'''Here, we are creating a manual schema using Pydantic'''
...
'''Here, we save the extracted invoice into a .JSON file'''
...
'''The code supports various cases:
1. Only PDF
2. PDF + Images
3. Images only
4. Using Llama Extract or PyPDF2'''
...
async def extract_invoices(pdf_directory: str, save_directory: str, use_llama: bool):
    ...
    if use_llama:
        ...
        if pdf_files:
            ...
            for response in data:
                extracted_invoice = {
                    ...
                    "Total_amount_in_words": response.data["Total_amount_in_words"],
                    "Payment_Details": {
        ...
        if image_files:
            '''Here, we process the images seperately'''
        ...
        '''Here, we process the PDFs seperately'''
